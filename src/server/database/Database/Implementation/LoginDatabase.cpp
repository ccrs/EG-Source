...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_ANTICHEAT_REPORTS_BY_AVERAGE, "SELECT id, guid, MAX(average) AS average, SUM(total_reports) AS total_reports, MAX(time) AS time, realmid, COUNT(*) AS days_flagged FROM account_anticheat_reports WHERE total_reports != 0 AND realmid = ? AND time >= ? GROUP BY guid ORDER BY average DESC LIMIT 10", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ANTICHEAT_REPORTS_BY_TOTAL, "SELECT id, guid, MAX(average) AS average, SUM(total_reports) AS total_reports, MAX(time) AS time, realmid, COUNT(*) AS days_flagged FROM account_anticheat_reports WHERE total_reports != 0 AND realmid = ? AND time >= ? GROUP BY guid ORDER BY total_reports DESC LIMIT 10", CONNECTION_SYNCH);
