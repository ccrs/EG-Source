...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INSTANCE, "SELECT character_instance.instance, permanent, map, difficulty, extendState, resettime FROM character_instance LEFT JOIN instance ON character_instance.instance = id WHERE guid = ?", CONNECTION_ASYNC);
    ...
    // Instance saves
    PrepareStatement(CHAR_INS_INSTANCE_SAVE, "REPLACE INTO instance (id, map, resettime, difficulty, completedEncounters, data) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_INSTANCE_DATA, "UPDATE instance SET completedEncounters=?, data=? WHERE id=? AND map=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PERM_BIND_BY_INSTANCE, "SELECT guid FROM character_instance WHERE instance = ? AND permanent = 1 AND extendState > 0", CONNECTION_SYNCH);
    ...
    // EG - Custom Settings
    PrepareStatement(CHAR_SEL_CUSTOM_SETTINGS, "SELECT customFlags FROM character_extended WHERE guid = ?", CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_DEL_ORPHANED_TRANSMOGRIFICATIONS, "DELETE FROM character_transmogrification WHERE NOT EXISTS (SELECT 1 FROM item_instance WHERE item_instance.guid = character_transmogrification.GUID)", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_EXISTING_SAME_ACCOUNT_CUSTOM_FLAGS, "SELECT 1 FROM character_extended LEFT JOIN characters ON character_extended.guid = characters.guid WHERE characters.account = ? AND character_extended.guid != ? AND (CAST(NULLIF(SUBSTRING_INDEX(SUBSTRING_INDEX(character_extended.customFlags, ' ', ?), ' ', -1), '') AS UNSIGNED) & ?) != 0 LIMIT 1", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_TOURNAMENT_RUN_BY_ID, "SELECT id, teamId, state, bossFinish, durationMs FROM tournament_run WHERE id = ?", CONNECTION_SYNCH);
