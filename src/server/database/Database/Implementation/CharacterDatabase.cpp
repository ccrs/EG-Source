...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_EXISTING_CHARACTER_SPELLS, "SELECT character_spell.spell FROM character_spell LEFT JOIN characters ON character_spell.guid = characters.guid WHERE character_spell.active = 1 AND characters.account = ? AND ((1 << (characters.race - 1)) & ?) != 0 AND character_spell.guid != ?", CONNECTION_ASYNC);
}
