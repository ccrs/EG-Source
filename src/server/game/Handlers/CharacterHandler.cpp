...
bool LoginQueryHolder::Initialize()
{
    ...
    // EG - Custom Settings
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CUSTOM_SETTINGS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_CUSTOM_SETTINGS, stmt);

    // EG - Transmogrification
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_TRANSMOGRIFICATIONS);
    stmt->setUInt32(0, lowGuid);
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    if (!bs_hair || bs_hair->Type != 0 || bs_hair->Race != _player->GetMasqueradeRace() || Gender(bs_hair->Sex) != _player->GetNativeGender())
        return;

    BarberShopStyleEntry const* bs_facialHair = sBarberShopStyleStore.LookupEntry(FacialHair);

    if (!bs_facialHair || bs_facialHair->Type != 2 || bs_facialHair->Race != _player->GetMasqueradeRace() || Gender(bs_facialHair->Sex) != _player->GetNativeGender())
        return;

    BarberShopStyleEntry const* bs_skinColor = sBarberShopStyleStore.LookupEntry(SkinColor);

    if (bs_skinColor && (bs_skinColor->Type != 3 || bs_skinColor->Race != _player->GetMasqueradeRace() || Gender(bs_skinColor->Sex) != _player->GetNativeGender()))
        return;
