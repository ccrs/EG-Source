...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPackets::Loot::LootItem& packet)
{
    ...
    Loot* loot = nullptr;
    uint8 lootSlot = packet.LootListID;
...
void WorldSession::HandleLootMoneyOpcode(WorldPackets::Loot::LootMoney& /*packet*/)
{
    ...
    if (player->HasCustomFlag(CUSTOM_AOELOOT_FLAGS, CUSTOM_FLAG_AOELOOT_ACTIVE) && player->GetLootFromAOELoot(guid))
    {
        ...
        WorldPackets::Loot::LootMoneyNotify lootMoneyNotify;
        lootMoneyNotify.Money = totalGold;
        lootMoneyNotify.SoleLooter = true; // "You loot..."
        SendPacket(lootMoneyNotify.Write());
    }
    else if (loot)
    {
