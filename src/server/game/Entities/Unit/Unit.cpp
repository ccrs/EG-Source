...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
        ...
        && (victim->GetTypeId() == TYPEID_PLAYER || !victim->ToCreature()->isWorldBoss())&& !victim->IsVehicle())
    {
        ...
        if (totalDamage > 0)
        {
            ...
            float const attackerMeleeSkill = GetMaxSkillValueForLevel();
...
{
    float z = destination.GetPositionZ() - (honorHover ? GetHoverOffset() : 0.f);
    if (z < destinationFloor - tolerance) // if really bellow ground, in air (caves,...)
        return true;
    if (z <= destinationFloor + tolerance) // if bellow ground or slightly above it, not in air
        return false;
...
bool Unit::SetDisableGravity(bool disable, bool updateAnimTier/* = true*/, bool /*temporally = false*/)
{
...
bool Unit::SetHover(bool enable, bool updateAnimTier /*= true*/, bool /*temporally = false*/)
{
    ...
    if (enable)
    {
        ...
        if (hoverHeight && std::fabs(GetPositionZ() - GetFloorZ()) < hoverHeight)
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player const* target) const
{
    ...
    {
        // EG - Race Masquerade: send masqueraded race/display to other players in the values update
        if (player
