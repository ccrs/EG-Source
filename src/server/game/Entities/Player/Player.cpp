...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    // EG: boosted days award honor at their own configured rate
    if (EG::IsBoostedDay())
        honor_f *= sWorld->getRate(RATE_HONOR_BOOST);
...
void Player::CastItemCombatSpell(DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto)
{
    ...
    if (canTrigger)
    {
        for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("entities.player.items", "Player::CastItemCombatSpell: Player '{}' ({}) has item '{}' (Entry: {}) with chance-on-hit effect in slot spellid_{} referencing unknown spell (ID: {})",
                    GetName(), GetGUID().ToString(), proto->Name1, proto->ItemId, i + 1, effectData.SpellID);
                continue;
            }
...
Item* Player::StoreNewItem(ItemPosCountVec const& pos, uint32 itemId, bool update, int32 randomPropertyId /*= 0*/, GuidSet const& allowedLooters /*= GuidSet()*/)
{
    ...
    if (item)
    {
        ...
        if (!pos.empty())
        {
            uint16 slot = pos.back().pos;
            item = GetItemByPos(slot >> 8, slot & 255);
            if (!item)
                return nullptr;
        }

        ObjectGuid pItemGuid = item->GetGUID();
...
bool Player::LoadFromDB(ObjectGuid guid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    _LoadAccountCharacters(holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_ACCOUNT_CHARACTERS));
...
void Player::StoreLootItem(uint8 lootSlot, Loot* loot, Optional<uint8> lootViewSlot/* = {}*/)
{
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (!lootViewSlot.has_value())
            lootViewSlot = GetLootViewSlot(loot, lootSlot);
        ...
        if (qitem)
        {
            ...
            {
                if (lootViewSlot.has_value())
                    SendNotifyLootItemRemoved(*lootViewSlot);
            }
            else
