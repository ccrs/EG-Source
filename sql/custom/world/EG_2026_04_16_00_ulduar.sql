...
-- Pool of Tar (33090): flags_extra = IMMUNITY_KNOCKBACK + CANNOT_ENTER_COMBAT + NO_XP
UPDATE `creature_template` SET `flags_extra` = 1073750080 WHERE `entry` = 33090;

-- Mimiron's Inferno + Reticle (33370/33369): hostile faction + trigger flags
...
-- Reticle (33369) carries Inferno (33370)
...
-- Spell 62909 must target the Mimiron's Inferno reticle (33369)
...
-- Spellclick on reticle (33369) -> Ride Vehicle (46598), creature-only
...
-- Mimiron's Inferno spellscript (62910) for target filtering
...
-- Thorim's Hammer + Reticle (33365/33364): hostile faction + trigger flags
...
-- Reticle (33364) carries Hammer (33365)
...
-- Spellclick on reticle (33364) -> Ride Vehicle (46598), creature-only
...
-- Spell 62911 must target the Thorim's Hammer reticle (33364)
...
-- Hodir's Fury + Reticle (33212/33108): hostile faction + trigger flags
...
-- Reticle (33108) carries Fury (33212)
...
-- Spell 62533 must target the Hodir's Fury reticle (33108)
...
-- Spellclick on reticle (33108) -> Ride Vehicle (46598), creature-only
...
-- Freya's Ward + Reticle (33367/33366): hostile faction + trigger flags
...
-- Reticle (33366) carries Ward (33367)
...
-- Spell 62906 must target the Freya's Ward reticle (33366)
...
-- Spellclick on reticle (33366) -> Ride Vehicle (46598), creature-only
...
-- Freya's add NPCs (Ward/Lashers/etc.)
...
--  Arachnopod Destroyer (34214): set VehicleId
UPDATE `creature_template` SET `VehicleId` = 399 WHERE `entry` = 34214;

-- Arachnopod Destroyer (34183): swap from SmartAI to EG C++ script
...
-- Arachnopod 10/25 (34183/34214): action bar spells (Spit, Web Wrap, Crush Armor)
...
-- Arachnopod 10/25 carries Clockwork Mechanic (34184)
...
-- Arachnopod 10-man (34183) spellclick: 46598 player-mount, 63313 creature-mount
...
-- Arachnopod 25-man (34214) spellclick
...
-- Supercharged (63528) must target a Storm Tempered Keeper (33699 or 33722)
...
-- "Can't Do That While Stunned" achievement criteria (FL hardmode): re-enable
...
-- Remove "disabled" flag from the same achievement criteria
DELETE FROM `disables` WHERE `sourceType`=4 AND `entry` IN (10090, 10091, 10423, 10425, 10422, 10424);

-- Mimiron Visual Bunny (33705): trigger flags
UPDATE `creature_template` SET `flags_extra` = 8384 WHERE `entry` = 33705;

-- Razorscale visuals (32866 / 33690): non-selectable + trigger flags
